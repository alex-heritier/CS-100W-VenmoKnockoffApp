 ...
 */
User::User(const string& username, int balance) :
...
string User::toString() const {
	 ...
	 */
	unsigned index = 0;
	for_each(funders.begin(), funders.end(),
			[&index, &info] (shared_ptr<FundSource> fund) {

				info << "[" << index << "]\n";
				if(typeid(*fund) == typeid(Card))
				{
					info << "Card Source\n";
				}
				else if(typeid(*fund) == typeid(Bank))
				{
					info << "Bank Source\n";
				}
				info << fund->toString() << "\n";
